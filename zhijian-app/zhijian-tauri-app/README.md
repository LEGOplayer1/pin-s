...
> **提示**：项目已配置 GitHub Actions CI/CD，push 代码并打 tag 即可自动构建 release。
...
## 🏗️ GitHub Actions 自动构建与发布

项目已配置 `.github/workflows/release.yml`，push 代码并打 git tag 即可自动构建并生成 release。

### 发布新版本步骤

```bash
# 1. 确认所有改动已 commit
git add . && git commit -m "feat: v0.2.0 release"

# 2. 打 tag（这会触发 GitHub Actions 自动构建）
git tag v0.2.0

# 3. 推送到 GitHub（tag 会自动创建 release）
...
- 安装 Rust + Node.js 环境
- 编译 Tauri 应用
- 生成 NSIS 安装包 + Portable 便携版
- 自动创建 GitHub Release，产物直接附加在 release 页面

> 💡 无需在本地安装 Rust！构建完全在 GitHub 云端完成。
